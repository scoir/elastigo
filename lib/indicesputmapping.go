...
func (c *Conn) PutMapping(index string, typeName string, instance interface{}, args map[string]interface{}, opt MappingOptions) error {
	...
	_, err = c.DoCommand("PUT", fmt.Sprintf("/%s/%s/_mapping", index, typeName), args, string(body))
	if err != nil {
		return err
	}
