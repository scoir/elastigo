...
type SearchDsl struct {
	...
	SortBody        interface{}              `json:"sort,omitempty"`
	FilterVal       *FilterWrap              `json:"filter,omitempty"`
	AggregatesVal   map[string]*AggregateDsl `json:"aggregations,omitempty"`
	SourceFilterVal map[string]interface{}   `json:"_source,omitempty"`
	FieldsVal       []string                 `json:"fields,omitempty"`
	ScriptFieldsVal map[string]*ScriptField  `json:"script_fields,omitempty"`
}
...
func (s *SearchDsl) SearchType(t string) *SearchDsl {
	s.args["search_type"] = t
	return s
}
...
func (s *SearchDsl) BodyFields(fields ...string) *SearchDsl {
	s.FieldsVal = fields
	return s
}

func (s *SearchDsl) ScriptField(name string, script *ScriptField) *SearchDsl {
	if s.ScriptFieldsVal == nil {
		s.ScriptFieldsVal = make(map[string]*ScriptField)
	}

	s.ScriptFieldsVal[name] = script
	return s
}
...
func (s *SearchDsl) SourceFilter(include, exclude []string) *SearchDsl {
	...
	if len(source) > 0 {
		s.SourceFilterVal = source
	}

	return s
}

func (s *SearchDsl) SortAny(val map[string]interface{}) *SearchDsl {
	s.SortBody = val
	return s
}
